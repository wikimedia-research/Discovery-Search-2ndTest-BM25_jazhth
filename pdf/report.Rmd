...
options(digits = 3, scipen = 500, encoding = 'UTF-8')
knitr::opts_chunk$set(warning = FALSE, message = FALSE, echo=FALSE)
path <- function(x) {
  if (grepl("pdf", getwd(), fixed = TRUE)) {
    return(file.path("..", x))
  } else {
    return(x)
  }
...
However, we then [realized](https://phabricator.wikimedia.org/T147008#2679631) that our analysis chain is sub-optimal for space-less language queries, which will break words on every characters for the plain field. Therefore, we ran a [second A/B test](https://phabricator.wikimedia.org/T147495) for Chinese, Japanese and Thai Wikipedias to test whether the new per-field BM25 builder is sufficiently worse with those languages. We are primarily interested in:

* __Zero results rate__, the proportion of searches that yielded zero results (smaller is usually better)
...
In `r figure_caps("zrr", display = "cite")`, we see that the test group that used BM25 with incoming links and pageviews as query-independent factors had a significantly lower zero results rate (ZRR). Smaller ZRR is usually better, but looking at the [PaulScore](#paulscore), [engagement](#engagement) and [First Clicked Result’s Position](#first_clicked_result’s_position), we doubt that it came at the cost of relevance and engagement with the results. `r figure_caps("zrr2", display = "cite")` shows that zhwiki had the largest ZRR difference between control and test group.
...
```
```{r zrr_caption, results = "asis", echo = FALSE}
print_caption(format_caption(figure_caps, "zrr"))
```

```{r zrr_eda2, dependson = "zrr"}
zrr_pages <- searches %>%
...
```
```{r zrr_caption2, results = "asis", echo = FALSE}
print_caption(format_caption(figure_caps, "zrr2"))
```

### PaulScore

In `r figure_caps("paulscore", display = "cite")`, we see that the test group had slightly lower PaulScores, which indicates that the results were less relevant. The difference is not significant when F = 0.9. This make sense because the smaller the value of scoring factor, the more weight put on the first few results, with lower ranking results counting for almost nothing. F=0.9 takes into account the broadest range of result rankings, and thus is less likely to change as dramatically. `r figure_caps("paulscore2", display = "cite")` shows that zhwiki had the largest PaulScore differences between control and test group.
...
```{r paulscores_eda, dependson = "paulscores"}
paulscores %>%
  ...
  theme(legend.position = "bottom") 
```
```{r paulscores_caption, results = "asis", echo = FALSE}
print_caption(format_caption(figure_caps, "paulscore"))
```
```{r paulscores_eda2, dependson = "paulscores"}
set.seed(777)
paulscores <- searches %>%
  ungroup %>%
  filter(clickthrough==TRUE) %>% 
...
paulscores %>%
  ...
  theme(legend.position = "bottom") 
```
```{r paulscores_caption2, results = "asis", echo = FALSE}
print_caption(format_caption(figure_caps, "paulscore2"))
```
...
```{r engagement_overall_eda, dependson = "engagement_overall"}
engagement_overall %>%
  ggplot(aes(x = 1, y = mean, color = `test group`)) +
  geom_pointrange(aes(ymin = lower, ymax = upper), position = position_dodge(width = 1)) +
  scale_color_brewer("Test Group", palette = "Set1", guide = guide_legend(ncol = 2)) +
  ...
  theme(legend.position = "bottom")
```
```{r engagement_overall_caption, results = "asis", echo = FALSE}
print_caption(format_caption(figure_caps, "engagement_overall"))
```
```{r engagement_overall_eda2, dependson = "engagement_overall"}
engagement_overall <- searches %>%
  filter(results=="some") %>% 
...
engagement_overall %>%
  ...
  theme(legend.position = "bottom")
```
```{r engagement_overall_caption2, results = "asis", echo = FALSE}
print_caption(format_caption(figure_caps, "engagement_overall2"))
```
...
```{r first_clicked_position_eda, dependson = "first_clicked_position"}
first_clicked %>%
  ggplot(aes(x = 1, y = mean, color = `test group`)) +
  geom_pointrange(aes(ymin = lower, ymax = upper), position = position_dodge(width = 1)) +
...
```
```{r first_clicked_position_caption, results = "asis", echo = FALSE}
print_caption(format_caption(figure_caps, "first_clicked"))
```

```{r first_clicked_position_eda2, dependson = "first_clicked_position", fig.height=10}
first_clicked <- searches %>%
  filter(results == "some" & clickthrough & !is.na(`first clicked result's position`)) %>%
  mutate(`first clicked result's position` = ifelse(`first clicked result's position` < 4, safe_ordinals(`first clicked result's position` + 1), "5th or higher")) %>%
...
first_clicked %>%
  ...
  theme(legend.position = "bottom",
        ...
        panel.border = element_rect(color = "gray30", fill = NA))
```
```{r first_clicked_position_caption2, results = "asis", echo = FALSE}
print_caption(format_caption(figure_caps, "first_clicked2"))
```
...
```{r dwell_time_eda, dependson = "aggregation_visits"}
clickedResults %>%
  split(.$`test group`) %>% 
  map_df(function(df) {
  ...
  theme(legend.position = "bottom")
```
```{r dwell_time_caption, results = "asis", echo = FALSE}
print_caption(format_caption(figure_caps, "dwell_time"))
```

```{r dwell_time_eda_wiki, dependson = "aggregation_visits", fig.height=10}
clickedResults %>%
...
```
```{r dwell_time_caption2, results = "asis", echo = FALSE}
print_caption(format_caption(figure_caps, "dwell_time2"))
```
...
```{r scroll_eda, dependson = "aggregation_visits"}
...
scroll_overall %>%
  ...
  theme(legend.position = "bottom")
```
```{r scroll_caption, results = "asis", echo = FALSE}
print_caption(format_caption(figure_caps, "scroll"))
```

```{r scroll_eda_wiki, dependson = "aggregation_visits"}
scroll_overall <- clickedResults %>%
  group_by(wiki, `test group`) %>%
  summarize(scrolls=sum(scroll), visits=n(), proportion = sum(scroll)/n()) %>%
  ungroup
...
scroll_overall %>%
  ...
  theme(legend.position = "bottom")
```
```{r scroll_caption2, results = "asis", echo = FALSE}
print_caption(format_caption(figure_caps, "scroll2"))
```
...
```{r reform_n_search_group, dependson = "reformulation"}
...
```
```{r reform_n_search_group_caption, results = "asis", echo = FALSE}
print_caption(format_caption(figure_caps, "reform_n"))
```
...
```{r reform_eda1, dependson = "reformulation"}
...
reformulation_counts %>%
  ...
  geom_text(aes(label = sprintf("%.2f%%", 100 * proportion),
                vjust = "bottom", hjust = "center"), nudge_x = 0.1)
```
```{r query_reformulations_prop_caption, results = "asis", echo = FALSE}
print_caption(format_caption(figure_caps, "query_reform_prop"))
```

```{r reform_eda1_wiki, dependson = "reformulation"}
reformulation_counts <- reform_times %>%
  group_by(wiki, `test group`) %>%
  summarize(`searches with query reformulations` = sum(as.numeric(unlist(lapply(n_reformulate, function(x) strsplit(x, ",")))) > 0),
            searches = sum(n_search),
...
reformulation_counts %>%
  ...
  facet_wrap(~ wiki, ncol = 3)
```
```{r query_reformulations_prop_caption2, results = "asis", echo = FALSE}
print_caption(format_caption(figure_caps, "query_reform_prop2"))
```

```{r reform_eda2, dependson = "reformulation", cache = TRUE}
...
reform_times %>%
  ...
  theme(legend.position = "bottom")
```
```{r query_reformulations_counts_caption, results = "asis", echo = FALSE}
print_caption(format_caption(figure_caps, "query_reform_counts"))
```

```{r reform_eda2_wiki, dependson = c("reformulation", "reform_eda2")}
reform_times %>%
  group_by(wiki, `test group`) %>%
  do(count_reformulation(.$n_reformulate)) %>%
  ggplot(aes(x = `query reformulations`, y = proportion, fill = `test group`)) +
  ...
  facet_wrap(~ wiki, ncol = 3)
```
```{r query_reformulations_counts_caption2, results = "asis", echo = FALSE}
print_caption(format_caption(figure_caps, "query_reform_counts2"))
```
